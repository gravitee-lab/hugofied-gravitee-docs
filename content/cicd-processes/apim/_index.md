...
* Then go to the  "Personal API Tokens" menu, and click the "Create new Token" button.
* Keep the value of your token, you will need it at each step.


## A kind of (magic) Release

You will release a given version of APIM, of version number `X.Y.Z`.
Whatever the relese verison number is, you will be in one of the two following cases :
* If `Z` is zero, then :
  * this release is a major release
  
* If `Z` is not zero, then this is


## Maven and Git release
